 	...
 	isset($_POST['password2']) || isset($_POST['phone']) || isset($_POST['plan'])){
		...
		!empty ($_POST['password2']) && !empty($_POST['phone']) && !empty($_POST['plan'])){
			...
			if ($_POST['password'] != $_POST['password2']){
				...
				$user = shell_exec("sudo -u www-data /www/Myshop/www/Server/add_fileDNS.sh $name");
				$user = shell_exec("sudo -u www-data /www/Myshop/www/Server/add_mailDirectory.sh $pseudo $pass");
				$user = shell_exec("sudo -u www-data /www/Myshop/www/Server/add_webUser.sh $pseudo $pass $name");
